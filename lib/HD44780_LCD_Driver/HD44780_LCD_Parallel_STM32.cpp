...
:   HD44780_LCD_Driver(), // we want to call the parent class constructor for this as it will initialize all of our variables for us
    _register_select_port(register_select_port),        // member initializer list, setting the const values for the various pins and ports used to manipulate the device
    _register_select_pin(register_select_pin),
    _read_write_port(read_write_port),
    _read_write_pin(read_write_pin),
...
void HD44780_LCD_Parallel_STM32::pulse_enable(void) {
    HAL_GPIO_WritePin(_enable_port, _enable_pin, (GPIO_PinState)HIGH); 
    HAL_Delay(INSTRUCTION_DELAY_MS);
    HAL_GPIO_WritePin(_enable_port, _enable_pin, (GPIO_PinState)LOW); 
    HAL_Delay(INSTRUCTION_DELAY_MS);
